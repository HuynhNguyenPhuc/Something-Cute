...
class Layer(object):
    ...
    def forward(self, x):
        ...
        elif (self.activation == 'softmax_minus_max'):
...
class NeuralNet(object):
    ...
    def add_linear_layer(self, w_shape, activation):
        ...
        elif (activation == 'softmax_minus_max'):
            self.layers.append(Layer(w_shape, 'softmax_minus_max', self.reg))
    ...
    def compute_loss(self, y, y_hat):
        ...
        """
        if y.ndim == 1:
            y_one_hot = create_one_hot(y, self.num_class)
        else:
            y_one_hot = y.copy()

        # [TODO 1.3]
        # Estimating cross entropy loss from y_hat and y 
        data_loss = -1./y.shape[0] * np.sum(y_one_hot * np.log(y_hat))

        # Estimating regularization loss from all layers
        reg_loss = 0.0
        for i in self.layers:
            reg_loss += np.sum(np.square(i.w))
        reg_loss = (self.reg/2.) * reg_loss
    ...
    def backward(self, y, all_x):
	    ...
	    # Use delta_dot_w_prev to compute delta factor for the next layer (in backpropagation direction)
            grad_w, delta_curr = layer.backward(x, delta_dot_w_prev)
...
def minibatch_train(net, train_x, train_y, cfg):
    ...
    iteration = np.ceil(cfg.num_train / cfg.batch_size).astype('int')
    for e in range(cfg.num_epoch):
        loss = 0.
        for i in range(iteration):
            start = i * cfg.batch_size
            end = cfg.num_train if (i + 1) == iteration else (i + 1)*cfg.batch_size
            train_temp_x = train_set_x[start:end]
            train_temp_y = train_set_y[start:end]
            all_x = net.forward(train_temp_x)
            y_hat = all_x[-1]
            i_loss = net.compute_loss(train_temp_y, y_hat)
            grads = net.backward(train_temp_y, all_x)
...
def star_classification():
    ...
    train_y = np.squeeze(train_y.astype(int))
    test_y = np.squeeze(test_y.astype(int))
    ...
    # Define hyper-parameters and train-related parameters
    cfg = Config(num_epoch=3000, learning_rate=0.01, batch_size=200, num_train=train_x.shape[0], visualize=False)
    ...
    net = NeuralNet(num_class, cfg.reg)
    net.add_linear_layer((train_x.shape[1], num_hidden_nodes), 'relu')
    net.add_linear_layer((num_hidden_nodes, num_hidden_nodes), 'relu')
    net.add_linear_layer((num_hidden_nodes, num_hidden_nodes), 'relu')
...
def iris_classification():
    ...
    # Define hyper-parameters and train-related parameters
    cfg = Config(num_epoch=3000, learning_rate=0.1, batch_size=200, num_train=train_x.shape[0], visualize=True)
